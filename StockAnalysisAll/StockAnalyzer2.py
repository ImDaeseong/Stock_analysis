...
class StockAnalyzer2:
    ...
    def analyze_Data(self, stock_code, window=14, rsi_buy_threshold=30, rsi_sell_threshold=70):
        ...
        # 매수 및 매도 신호 생성
        df['Signal'] = ''
        df.loc[df['RSI'] < rsi_buy_threshold, 'Signal'] = '매수'
        df.loc[df['RSI'] > rsi_sell_threshold, 'Signal'] = '매도'

        # 신호가 발생한 시점만 선택
        df_signals = df[df['Signal'].isin(['매수', '매도'])].copy()  # copy()로 데이터프레임 복사
